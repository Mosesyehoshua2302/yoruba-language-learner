...
Requires Node 18+. The app works fully offline: every screen reads/writes through
`src/state/store.tsx`, which keeps a `localStorage` cache for instant loads and offline use.

### Persistent storage (local Postgres)

By default that cache is the only copy of your progress, and it's scoped to the browser origin
(protocol+host+**port**) — if Vite picks a different port on a later run because 5173 was busy,
the app looks like it "forgot" everything. To make progress durable regardless of ports, browser
profile, or dev-server restarts, `server/` is a small local Express API that mirrors the state
into a local Postgres database; the frontend syncs to it automatically once it's reachable and
falls back to the localStorage cache when it isn't.

One-time setup:

```bash
cd server && cp .env.example .env   # fill in your local Postgres user/password
cd ..
npm run db:setup   # creates the `yoruba_learner` database + table
```

Then either run both processes with one command:

```bash
npm run dev:full    # vite + the state server together
```

or in two terminals: `npm run server` and `npm run dev`. The frontend (`src/lib/api.ts`) talks to
the server directly at `http://localhost:8787`, independent of whatever port Vite lands on; the
server sends permissive CORS headers so this works in dev, `vite preview`, or a production build.
Data lives in the `learner_state` table (one row, JSONB blob) — inspect it any time with
`psql -d yoruba_learner -c 'table learner_state'`. If the server isn't running, the app keeps
working off the last-synced localStorage copy.

To host it on AWS instead (S3/CloudFront frontend + Cognito/API Gateway/Lambda/DynamoDB
state-sync backend), see [`infra/README.md`](infra/README.md) — TypeScript CDK,
deployed with `cdk deploy --all`. That's a separate, optional deployment target; the local
Postgres server above is unrelated to it.
...
src/lib/storage.ts       localStorage cache (versioned) + content reconciliation
src/lib/api.ts           client for the local state server (server/)
src/lib/__tests__/       unit tests (vitest; also run standalone via tsx)
src/state/store.tsx      React context + reducer wiring the modules to the UI, server sync
...
server/                  local Express + Postgres API mirroring learner state (see above)
```
