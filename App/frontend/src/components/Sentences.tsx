...
import { getSentences } from "../lib/content";
import type { VocabPair, OppositePair } from "../types";
...
export function Sentences() {
  ...
  const category =
    sentences.categories.find((c) => c.id === activeCat) ??
    sentences.categories[0];
    ...
    <div className="space-y-6">
      <div className="card flex gap-3 border-gold-300/70 bg-gold-50/70 p-4 dark:border-gold-500/30 dark:bg-gold-900/15">
        ...
        <div className="min-w-0 text-sm text-gold-900/90 dark:text-gold-100/85">
          <p className="font-semibold text-gold-800 dark:text-gold-300">
            Informal phrasebook — unverified
          </p>
      ...
      {category && (
        <div
          key={category.id}
          className="view-transition grid grid-cols-1 gap-3 sm:grid-cols-2"
        >
          {category.items.map((item) => {
            ...
            return (
              <div key={item.id} className="relative">
                ...
                >
                  <div
                    className={`flip-card h-full ${isFlipped ? "is-flipped" : ""}`}
                  >
      ...
      <p className="text-xs text-ink-400 dark:text-ink-600">
        Source: {sentences.meta.title} ({sentences.meta.note}). Reference only —
        not part of the gated progression or spaced-review system.
      </p>
    </div>
  );
}
