    ...
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-40 border-b border-ink-200/70 bg-ink-50/85 backdrop-blur-md transition-colors dark:border-ink-800/80 dark:bg-ink-950/85">
        <div className="mx-auto max-w-4xl px-4">
          <div className="flex items-center gap-3 pt-2.5">
            <h1 lang="yo" className="font-display text-xl font-semibold tracking-tight">
              Yorùbá Yé Mi
            </h1>
            <div className="ml-auto flex items-center gap-2.5">
              ...
              >
                <ProgressRing value={level.xpIntoLevel / level.xpForNextLevel} size={30} strokeWidth={3}>
                  <span className="text-[11px] font-bold tabular-nums">{level.level}</span>
                </ProgressRing>
              ...
              >
                ...
                <span className="text-sm font-semibold tabular-nums">{state.streak.days}</span>
              </div>
              <ThemeToggle />
            </div>
          </div>
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-6">
        <div key={view.name} className="view-transition">
          ...
          {view.name === 'lesson' && <Lesson chapterId={view.chapterId} setView={setView} />}
          {view.name === 'review' && <Review setView={setView} />}
          {view.name === 'assessment' && <Assessment chapterId={view.chapterId} setView={setView} />}
          {view.name === 'sentences' && <Sentences />}
        </div>
      </main>
      <footer className="mx-auto w-full max-w-4xl px-4 pb-8 text-xs text-ink-400 dark:text-ink-600">
        Content: {content.meta.title} — {content.meta.author} ({content.meta.license})
      </footer>
      <Celebrations />
    </div>
  );
